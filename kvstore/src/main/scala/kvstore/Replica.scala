...
class Replica(val arbiter: ActorRef, persistenceProps: Props) extends Actor {
  ...
  val leader: Receive = {
    ...
    case Insert(key, value, id) => {
      ...
      persistRepeaters += (id -> context.system.scheduler.schedule(0 millis, 100 millis, persistence, Persist(key, Some(value), id)))
    ...
    case Remove(key, id) => {
      ...
      persistRepeaters += id -> context.system.scheduler.schedule(0 milli, 100 millis, persistence, Persist(key, None, id))
      failureGenerators += id -> context.system.scheduler.scheduleOnce(1 second) { self ! GenerateFailure(id) }
    }
